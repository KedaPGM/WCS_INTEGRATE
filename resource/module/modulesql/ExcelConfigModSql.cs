...
namespace resource.module.modulesql
{
    public class ExcelConfigModSql : BaseModSql
    {
        ...
        public List<T> GetExcelData<T>(string path)
        {
            List<T> list = new List<T>();
            DataTable dt = DataTableExtend.TransformExcelToDataTable(path);
            if (!mSql.IsNoData(dt))
            {
                list = dt.ToDataList<T>();
            }
            return list;
        }
        ...
        public void SaveToExcel(SaveFileDialog saveFileDialog)
        {
